...
echo ${da1}月${da2}日
python3 program.py "７時になりました。"$da"の休講情報をお知らせします。" $username $password &&  \
...
awk -F'>' '{print $2}' | awk -F'<' '{print $1}' | xargs -I@ echo @ | xargs -L 4 |xargs -I@   python3 program.py "$da @" $username $password  && \
python3 program.py $da"の休講情報は以上です。"  $username $password
